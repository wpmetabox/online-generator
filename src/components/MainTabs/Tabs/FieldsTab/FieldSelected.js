...
const FieldSelected = (props) => {
  ...
  if ('divider' === type) {
    return (
      <div
        className={`og-item og-item--${type} og-collapsible${
          expanded ? ' og-collapsible--expanded' : ''
        } ${
          props.dragAndDrop &&
          props.dragAndDrop.draggedTo === index &&
          'dropArea'
        }`}
      >
        <input
  ...
  return (
    <div
      className={`og-item og-item--${type} og-collapsible${
        ...
        props.dragAndDrop && props.dragAndDrop.draggedTo === index && 'dropArea'
      }`}
    >
      <input
        ref={props.register}
      ...
      <div class="og-item__sort">
        <button
          ...
          class="og-item__up"
          title="Move up"
          onClick={() => props.changePosition(props.index, 'up')}
        >
          {arrowUpIcon}
        </button>
        <button
          ...
          class="og-item__down"
          title="Move down"
          onClick={() => props.changePosition(props.index, 'down')}
        >
          {arrowDownIcon}
        </button>
      </div>
...
const Header = (props) => {
  ...
  return (
    ...
    >
      <div className="og-item__title" id={`og-item__title__${props.index}`}>
        {props.name || ucfirst(props.type)}
      </div>
      <div className="og-item__actions">
        <span className="og-item__type">{props.type}</span>
        <span
          className="og-item__action og-item__action--remove"
          title="Remove"
          onClick={remove}
        >
          {trashIcon}
        ...
        <span
          className="og-item__action og-item__action--duplicate"
          title="Duplicate"
          onClick={duplicate}
        >
          {copyIcon}
        ...
        <span
          className="og-item__action og-item__action--toggle"
