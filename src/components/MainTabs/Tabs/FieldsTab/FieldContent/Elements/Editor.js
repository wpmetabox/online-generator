...
const Editor = (props) => {
    const [optionalList, setAttributes] = useState(props.data['editor']);
    const customData = ADVANCED_ADDITIONAL['editor'] ;
    ...
    return (
          ...
          <tbody>
            {
              optionalList?.map((item, index) => (
                <AdvancedAdditionalItem data={item} key={index} index={index} register={props.register} name={`fields-${props.index}`} type='editor'/>
              ))
            }
          </tbody>
        </table>
        ...
        <input type='hidden' name={`fields-${props.index}-editor`} value={optionalList?.length} />
      </div>
    )
}
